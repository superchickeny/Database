...
function getCellStyle(val: unknown): React.CSSProperties {
  const base: React.CSSProperties = { fontFamily: "'JetBrains Mono', monospace", fontSize: 12, whiteSpace: 'nowrap', padding: '8px 16px' }
  if (val === null || val === undefined) return { ...base, color: '#50505f', fontStyle: 'italic' }
  if (typeof val === 'number') return { ...base, color: '#4ade80' }
  if (typeof val === 'boolean') return { ...base, color: '#60a5fa' }
  return { ...base, color: '#e8e8ec' }
}
                  ...
                  {rows.map((row, i) => (
                    ...
                    >
                      ...
                      {columns.map(col => {
                        ...
                        return (
                          <td key={col} style={getCellStyle(val)}>
                            {getCellDisplay(val)}
                          </td>
                        )
                      })}
