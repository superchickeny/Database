...
function HomePage({onRunQuery, queries, setQueries}: HomePageProps) {
    ...
    <div className="flex-1 overflow-y-auto" style={{ padding: 32 }}>
      <div className="flex flex-col gap-7">
        ...
        <div className="grid grid-cols-3 gap-3">
          {[
            { label: 'Session Queries', value: queries.length, color: '#2dd4bf' },
            { label: 'Avg response', value: (queries.reduce((sum, b) => sum + b.timeInMs, 0) / queries.length).toFixed(2) + "ms", color: '#4ade80' },
            { label: 'Rows fetched', value: queries.reduce((sum, b) => sum + b.rowCount, 0), color: '#fbbf24' },
          ].map(s => (
            <div
              key={s.label}
            ...
            <tbody>
              {queries.map((r, i) => (
                ...
                >
                  ...
                  <td style={{ fontSize: 12, color: '#888896', padding: '9px 12px', ...SANS }}>{r.rowCount}</td>
                  <td style={{ fontSize: 12, color: '#888896', padding: '9px 12px', ...SANS }}>{r.timeInMs}ms</td>
